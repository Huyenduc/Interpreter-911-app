...
const VideoCallScreen = () => {
  ...
  const _onMuteButtonPress = () => {
    twilioVideo.current
      ...
      .then((isEnabled: any) => dispatch(propsEnableAudio(isEnabled)));
  };
  ...
  const _onDisableVideoButtonPress = async () => {
    twilioVideo.current.setLocalVideoEnabled(!props.isVideoEnabled).then((isEnabled: any) => {
  ...
  return (
    <View style={styles.callContainer}>
      {props.status === 'connected' ? (
        <View style={{ flex: 1, marginBottom: '10%' }}>
          ...
          {props.isVideoEnabled ? (
            <TwilioVideoLocalView enabled={props.status === 'connected'} applyZOrder={true} style={styles.localVideo} />
          ) : (
      ...
      <View style={styles.optionsContainer}>
        <TouchableOpacity
          style={[styles.button, { backgroundColor: '#ed3b2d', marginLeft: 10 }]}
